...
export default function AuthModal({ open, onClose, intent }) {
  const [email, setEmail] = useState("");
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);
  if (!open) return null;

  // Compute where to send users after auth
  const redirectTarget = useMemo(() => {
    if (intent) return intent; // e.g., "/list"
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${origin}/api/auth/callback?redirect=${redirectParam}`,
        },
      });
    } catch (e) {
      console.error(e);
  ...
  const signInWithEmail = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
          emailRedirectTo: `${origin}/api/auth/callback?redirect=${redirectParam}`,
        },
      });
