...
export default function AuthModal({ onClose }) {
  ...
  async function handleEmail(e) {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email: email.trim(),
        options: { emailRedirectTo: `${window.location.origin}/auth/callback?redirect=/` },
      });
  ...
  async function handlePhone(e) {
    ...
    if (!value) { setError("Please enter your phone number."); return; }
    if (!value.startsWith("+")) { setError("Use international format like +15551234567."); return; }

    setLoading(true);
    try {
      const { error } = await supabase.auth.signInWithOtp({ phone: value });
      if (error) throw error;
      // Go to code entry page immediately
      window.location.href = `/verify-sms?phone=${encodeURIComponent(value)}&redirect=/`;
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
          ...
          <form onSubmit={handlePhone} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-500"
              />
              <p className="mt-1 text-xs text-gray-500">Use international format (e.g., +15551234567).</p>
            </div>
            <button type="submit" disabled={loading}
              className="w-full rounded-md bg-brand-500 px-4 py-2 font-semibold text-white hover:bg-brand-600 disabled:opacity-60">
        ...
        <p className="mt-4 text-center text-xs text-gray-500">
          By continuing, you agree to our <a href="/terms" className="text-brand-500 hover:text-brand-600">Terms</a> and{" "}
